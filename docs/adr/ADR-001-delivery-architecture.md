...
`//go:embed` は対象ディレクトリに埋め込み可能なファイルが 1 つも無いとコンパイルエラーになる。`backend/static/` は現在 git 未追跡の空ディレクトリであり、クリーンチェックアウトでは存在すらしない。
...
したがって **`all:` プレフィクスを付けたうえで `backend/static/.gitkeep` をコミットする**。ダミーの `index.html` を置く必要はない。
